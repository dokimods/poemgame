...
init python: #This whole block runs when DDLC is started (as opposed to when the poem minigame is called)
    ...
    full_wordlist = []
    # PoemWord.load_from_txt("poemwords.txt")
    PoemWord.load_from_json("poemword.json")
    ...
    #         x = line.split(',')
    #         full_wordlist.append(PoemWord(word=x[0], sPoint=float(x[1]), nPoint=float(x[2]), yPoint=float(x[3])))
...
label poem_old(transition=True):
